...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!sentenceCount || typeof sentenceCount !== "number") {
      return res.status(400).json({ error: "Invalid sentence count" });
    }
...
Words to include: ${words.join(", ")}`
    ...
    let response = await callAnthropic("claude-3-sonnet-20240229");

    // Fallback model (Haiku)
    if (!response.ok) {
      console.warn("⚠️ Sonnet failed — falling back to Haiku");
      response = await callAnthropic("claude-3-haiku-20240307");
    }
    ...
    if (!story) {
      return res.status(500).json({ error: "No story returned from Anthropic" });
    }
